...
if which /opt/aws/bin/ec2-metadata &>/dev/null ; then
STACK_NAME=$(aws ec2 describe-tags \
	--filter Name=resource-id,Values="$(/opt/aws/bin/ec2-metadata | grep instance-id | awk '{ print $2}')" \
...
INSTANCE=$(/opt/aws/bin/ec2-metadata | grep instance-id | awk '{ print $2}')
