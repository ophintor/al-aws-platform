...
def handler(event, context):
    ...
    try:
        ...
        s3 = setup_s3_client()
        sync_service_catalog(s3, artifact_data, job_id)
...
def sync_service_catalog(s3, artifact, jobid):
...
def update_portfolio(objPortfolio, objMappingFile, bucket):
    ...
    # TODO: Can't the remove and add Tags be done at the same time?
...
def list_portfolios():
    ...
    while not done:
        if nextmarker:
            # TODO: nextmarker is not documented, the documented parameter is PageToken
            # TODO: do we need this condition? if null it retrieves the first page anyway...
...
def create_provisioning_artifact(objProduct, portfolioid, productid, s3objectkey):
    ...
    artifactDetails = client.list_provisioning_artifacts(ProductId=productid)
    latestArtifact = artifactDetails['ProvisioningArtifactDetails'][-1]
    latestVersion = float(latestArtifact['Name'])
...
def remove_portfolio_share(lst_accounts, portfolioid):
    ...
    # TODO: if the list is empty why are we deleting the account from it?
...
def list_portfolio_shares(portfolioid):
    ...
    while not done:
        if nextmarker:
            # TODO: nextmarker is not documented in parameters but is documented as NextPageToken in the response....
...
def put_bucket_policy(policy, s3bucket):
    ...
    :return: None
    """
    # TODO: Why this doesn't uses the setup_s3_client ?
    s3_client = boto3.client('s3')
